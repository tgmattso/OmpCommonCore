...
!           simplification in C (Helen He, November 2023)
  ...
  contains 
     ...
     INTEGER FUNCTION testpoint(creal, cimag)
...
!$OMP PARALLEL DO DEFAULT(shared) PRIVATE(creal,cimag,j) FIRSTPRIVATE(eps) reduction(+:numoutside)

  DO i = 1, NPOINTS
  DO j = 1, NPOINTS
     creal = -2.0+2.5*DBLE(i-1)/DBLE(NPOINTS)+eps
     cimag = 1.125*DBLE(j-1)/DBLE(NPOINTS)+eps
     numoutside = numoutside + testpoint(creal, cimag)
  ENDDO
  ENDDO
!$OMP END PARALLEL DO
